...
Net USD = `AmountOrderNetUSD + BackOrderedExtendedAmount` — the cube's stored net amount plus
the back-ordered value, which the cube holds outside the net (2 lines carry it; Cognos includes
them). Net EUR is the cube's native `AmountOrderNetEUR + BackOrderedExtendedAmountEUR` for
EUR-currency companies; USD-local companies (00010/00030), which the cube does not rate into
EUR, convert the USD total ÷ the month-end EUR/USD rate A of the GL date (the cube's own
`Currency Rates` table, one row per month end) — the same monthly conversion Cognos applies,
so the column is populated on every line. Consequence, accepted: amounts differ from Cognos at
rate level — Cognos re-converts every line at its own monthly rate.
...
Shipments Net USD 53,817,847 → 53,953,620 (+0.25%) and Net EUR 48,290,178 → 48,283,201 (−0.01%),
both the rate class in FINDINGS; Forecast
