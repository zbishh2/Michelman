// ============================================================================
// Report 22 - "CM - Information 2020 - Future" - FORECAST (sheet 3 of 6)
// COMMENTED MASTER. The shipped file is "Forecast.m" (comment-free, repo rule
// CLAUDE.md). Maintain the two in parallel; the code must stay byte-identical.
//
// Cognos: FORECAST (F3460 SA, RELOAD_KEY N) x ITEM x customer, 70-bulk list,
// requested date from sysdate's month start to month-end of sysdate+450 days,
// positive forecast only. Source here is SSASPROD / BIQLTabular 'FactForecast',
// whose rows already equal that set (every positive row is ForecastType SA /
// DWSource 1 / Bypass N / OrderType FC) - no extra decode predicate needed.
//
// Tie-out (PROBE/FINDINGS.md): 2,304 rows = Cognos 2,276 + 28 rows dated the
// 1st of the current month. Cognos's lower bound carries sysdate's clock time
// so rows on the 1st at 00:00 fall out (an Oracle artefact, 1.2%); this build
// is first-of-month inclusive on purpose. Quantities exact (KG within 0.01 on
// 141 rows - Cognos's constant factor).
//
// Revenue Business Unit is OMITTED: Cognos sources it from the legacy sales
// dimension and nothing on FactForecast or its customer resolves it (open
// question to Dave). TM Name is NOT in this query: FactForecast's own
// territory manager is set for FC-group customers only (730 of 2,304 rows
// blank), while Cognos stamps the customer's commission TM. The model column
// Forecast[TM Name] is a LOOKUPVALUE into 'TM Assignment' (EDW
// BIQL.TbTM_Max_Assignment, FC-group TM else CS-group TM, keyed on Customer
// Code) - the documented field-level step down the source ladder; it matches
// Cognos on all 41 forecast customers (PROBE/11e_edw_customer_tm.csv).
// ============================================================================
let
    Raw = AnalysisServices.Database(
        "SSASPROD",
        "BIQLTabular",
        [
            Query = "
EVALUATE
// The 70-bulk list, verbatim.
VAR Bulks = { ""161017CX"", ""161190PX"", ""171143PX"", ""171228PX.E"", ""181020CX.E"", ""181136IX"", ""181192IX"", ""181193EU.E"", ""191011CX"", ""191026CX.E"", ""191245PX"", ""23409A"", ""ABEX2525"", ""APT10"", ""APT11"", ""DMAEMA"", ""EMA3065"", ""ET2012.E"", ""ET2022.E"", ""ET4075.E"", ""ET440.E"", ""FERSUL7W"", ""HP1432AT"", ""HP1632"", ""MD4020"", ""MD4020C"", ""MD4020S"", ""MD4021"", ""MD4021C"", ""MD4021S"", ""MD4022"", ""MD4022C"", ""MD4023"", ""MD4023C"", ""MDU20"", ""MDU2012.E"", ""MDU2012B.E"", ""MDU4075.E"", ""MDU4075B.E"", ""MDU440.E"", ""MDU440B.E"", ""MPEG2000"", ""MW40504"", ""MW40514"", ""NP4LF"", ""NP4LF.S"", ""OMS"", ""PUD1.E"", ""STODSO"", ""U1001"", ""U101"", ""U201"", ""U2022"", ""U2022EU.E"", ""U2023"", ""U204"", ""U204EU.E"", ""U470"", ""U501"", ""U501B"", ""U502"", ""U502.E"", ""U502X1.E"", ""U601"", ""U701"", ""U802"", ""U802.E"", ""WAV501"", ""WD40"", ""WD40T"" }
// First of the current month, inclusive - Cognos excludes 00:00 on the 1st by accident (28 rows).
VAR WindowStart = DATE ( YEAR ( TODAY () ), MONTH ( TODAY () ), 1 )
// Month end of today+450 days, as Cognos.
VAR WindowEnd = EOMONTH ( TODAY () + 450, 0 )
VAR Forecasts =
    FILTER (
        FactForecast,
        TRIM ( RELATED ( 'Item Branch'[Item Bulk] ) ) IN Bulks
            && FactForecast[RequestedDate] >= WindowStart
            && FactForecast[RequestedDate] <= WindowEnd
            // Cognos CURRENT_FORECAST > 0.
            && FactForecast[QuantityForecast] > 0
    )
RETURN
    SELECTCOLUMNS (
        Forecasts,
        // Leading zeros; stays text.
        ""Company Code"", FactForecast[Company],
        ""Branch Plant"", TRIM ( FactForecast[BusinessUnit] ),
        ""Global Bulk Item"", FactForecast[Global Bulk],
        ""Bulk Item"", FactForecast[Bulk Item],
        ""2nd Item Number"", FactForecast[ItemNum2nd],
        ""Item Description 1"", RELATED ( 'Item Branch'[Item Num 2nd Desc] ),
        // Cognos prints '-' for blank (every row).
        ""Item Description 2"", RELATED ( 'Item Branch'[Description 2] ),
        ""Requested Date"", FactForecast[RequestedDate],
        // TM Name is a model column looked up from 'TM Assignment' (EDW) - see header. Revenue Business Unit omitted - see header.
        ""Current Forecast (Line)"", FactForecast[QuantityForecast],
        ""Primary UOM"", FactForecast[UOM Primary],
        ""Current Forecast LB (Line)"", FactForecast[QuantityForecastLB],
        ""Current Forecast KG (Line)"", FactForecast[QuantityForecastKG],
        ""Date"", FactForecast[RequestedDate],
        ""Year"", YEAR ( FactForecast[RequestedDate] ),
        ""Month"", MONTH ( FactForecast[RequestedDate] ),
        ""Customer Code"", FactForecast[AddressNum],
        // FactForecast's customer is an address-book member; 'Address' needs the quotes.
        ""Customer Name"", RELATED ( 'Address'[Address Name] ),
        ""Global Parent Name"", RELATED ( 'Address'[Global Parent Desc] ),
        ""Chemist Name"", RELATED ( 'Item Branch'[Chemist Name] )
    )
"
        ]
    ),
    Data = Table.TransformColumnNames(
        Raw,
        each if Text.StartsWith(_, "[") and Text.EndsWith(_, "]")
            then Text.Middle(_, 1, Text.Length(_) - 2)
            else _
    )
in
    Data
