# Report 07 — Ivan SK 2023  (BUILD)

Cognos report name: **`1 - Ivan SK 2023`**. Standalone → its own single-page-per-tab PBIP
(5 pages). Rebuild target: reproduce the Cognos report as closely as possible (parity-now).

**Structural CLONE of report 09 — Ivan FC 2023.** SK and FC are the same report (same
SELECT / JOIN / GROUP BY / UOM conversions / REGION decodes on every page); they differ
**only in WHERE-clause filter literals**. This build was produced by cloning 09 and swapping
in SK's literals. **SK-vs-FC deltas (all 5 pages):**
- **Bulk-item whitelist** is a different family (SK: PR3460 / PR5980I / PR5985 / DPI* / MF* /
  MP* / 24xxxPX / 25xxxNX …). Page 1 & 3 share a **99-entry** list; page 4 uses a **13-entry**
  HP list; page 5 uses a **21-entry** HP list. Page 2's component filter is an **8-entry** list
  (PR3460, PR3460.E, PR5980I{,.E,.S}, PR5985{,.E,.S}).
- **Branch plants**: SK **adds the Americas plants CINC/CIN2/CIN4** to the FC APAC/EMEA set on
  pages 1, 4, 5 and on page 3's SO7 open-order filter (which SK further widens with **BARC/CIND/
  CINR**). Page 3's `Item_Information8` (II) branch list stays at the 6 APAC/EMEA plants (as in FC).
- **Page 2 Start-Date floor**: `>= 2026-03-01` (FC used `2025-11-01`).
The FC REGION decodes already map CINC/CIN2/CIN4→Americas (p1/p2) / →Americas (p4/p5), so they
cover SK's extra plants unchanged.
...
`FINAL - for handover\1 - Ivan SK 2023.pbix` was inspected on 2026-07-09: it contains **no
`Last Refreshed` table and no `card` visual**. Someone must open `PBIP\1 - Ivan SK 2023.pbip` in
...
| (b) Cognos render-DISTINCT missing → `Work_Orders` over-counts | **FIXED** | `Work_Orders.m:67` `SELECT DISTINCT` (also `Safety_Stock_HP.m:36`, `Sales_Order_Summary.m:99`) |
| (c) F4211-only sales history decays as lines purge to F42119 | **N/A — does not apply, and adding it would be a bug** | This report has no sales-*history* page. `Sales Order Summary` reads **open** orders from F4211, filtered `SDNXTR NOT IN ('570','580','620','999')` at `Sales_Order_Summary.m:163`. Purged lines are out of scope *by construction*. `Generated SQL (Cognos - raw).sql` has **0** `F42119` references, and so does every `.m` here. |
...
**FC** whitelist (**25 literals, 15 distinct**), further evidence that the SK report was cloned
from the FC report inside Cognos.
...
Set-differencing the active Cognos lists against the `.m` `IN (...)` lists gives an exact match with
**zero prohibited items leaked**:

| Query | Active list (Cognos) | `.m` list | Verdict |
|---|---|---|---|
| Inventory | 99 literals, 99 distinct | 99 | match |
| Sales Order Summary | 99 literals, 99 distinct | 99 | match |
| Inventory - New (HP) | 13 literals, 13 distinct | 13 | match |
| Safety Stock - New (HP) | 21 literals, 21 distinct | 21 | match |
| *(prohibited, all three)* | *25 literals, 15 distinct* | *not applied* | correct |

Had the prohibited filter been ANDed in, these pages would return **0 rows**.

### Hard-coded date literal — stale, not breaking

The Work Orders page carries `Start Date >= 2026-03-01` (report 09's twin uses `2025-11-01`), copied
...
It does silently **widen** the history window's meaning as time passes: what was "the last ~4 months"
...
- **Whitelist:** the long `Bulk Item IN (...)` list (SK: **99 entries** on pages 1 & 3, **13** on
  page 4, **21** on page 5) is kept **verbatim** in each `.m`, copied character-for-character from
  the SK generated SQL. The Report XML also carries a second `use="prohibited"` Bulk-Item filter
  (25 literals / 15 distinct) on three queries; it does **not** survive into the generated SQL, so
  it is **not** applied (we match the generated SQL, the source of truth for what ran).
...
|---|---|---|---|---|---|---|
| 1 | Inventory | Inventory | `Inventory.m` | `Inventory` | 16 | **303** |
| 2 | Work Order | Work Orders | `Work_Orders.m` | `Work_Orders` | 22 | **213** |
| 3 | Sales Orders | Sales Order Summary | `Sales_Order_Summary.m` | `Sales_Order_Summary` | 29 | **55** |
| 4 | Inventory HP | Inventory - New | `Inventory_HP.m` | `Inventory_HP` | 11 | **152** |
| 5 | Safety Stock HP | Safety Stock - New | `Safety_Stock_HP.m` | `Safety_Stock_HP` | 9 | **75** |

Validation source: `Ivan Reports/Ivan SK 2023.xlsx` (one sheet per page; row counts above verified by
dumping the sheet XML). Row counts above are the targets. No slicers/prompts/on-page plaintext exist
in this report (pure list pages).

---

## Page 1 — Inventory  (`Inventory.m`, 16 cols)
Grain: lot/location. Flat `SELECT + GROUP BY` over F4102/F554101/F41021/F4101.
Filter: Branch in (AUBA,AUB2,SING,SNG4,MUM3,SHAN,CINC,CIN2,CIN4); on-hand>0; Bulk in the 99-entry whitelist.
...
Row-level filters: Component 2nd item in (PR3460, PR3460.E, PR5980I, PR5980I.E, PR5980I.S, PR5985,
PR5985.E, PR5985.S); Issued+Ordered>0; Start Date ≥ 2026-03-01; 2nd Item `NOT LIKE '%-%'`;
WAUOM in (LB,KG); WASRST not in (MM).
...
`os` open-order (SO7) filters: SDLNTY='S', SDPQOR>0, SDNXTR not in (570,580,620,999), SDMCU in the **12**
branches (AUBA,AUB2,SING,SNG4,MUM3,SHAN,CINC,CIN2,CIN4,BARC,CIND,CINR). Note: `II` stays at the 6 APAC/EMEA
plants, so the LEFT-JOIN-as-inner-join on Branch_Plant keeps only plants present in BOTH (Cognos quirk).
Final `II.Bulk_Item` filter = the SK 99-entry whitelist.
...
in (T,B,Q,H); Branch in (AUBA,AUB2,SING,SNG4,MUM3,SHAN,CINC,CIN2,CIN4); Bulk in the 13-entry HP whitelist.
...
Branch in (AUBA,AUB2,SING,SNG4,MUM3,SHAN,CINC,CIN2,CIN4); Bulk in the 21-entry HP whitelist.
...
- [ ] Server = ODSPROD; refresh succeeds (all 5 native queries fold, no "Incorrect syntax near 'WITH'"
      / no ORDER-BY error).
- [ ] Row counts: Inventory **303**, Work Order **213**, Sales Orders **55**, Inventory HP **152**,
      Safety Stock HP **75**.
...
- [ ] **Open `PBIP\1 - Ivan SK 2023.pbip` in Power BI Desktop, re-save, publish** — otherwise the
...
## Self-check (intake)
Cloned from 09; only WHERE-clause literals were swapped, so SELECT/JOIN/GROUP BY structure is
byte-identical to FC. All 5 `.m` re-verified: **paren-balanced** (whole-file `(`=`)`: Inventory
102/102, Work_Orders 189/189, Sales_Order_Summary 139/139, Inventory_HP 77/77, Safety_Stock_HP
42/42), **no `WITH`** and **no `ORDER BY`** in the SQL (only in header comments), string literals
balanced. Every swapped `IN (...)` list character-matches the SK generated SQL (Inventory bulk 99,
WO component 8, Sales bulk 99, Inventory_HP bulk 13, Safety_Stock_HP bulk 21; branch lists per page).
The 5 tmdl partitions were rebuilt from these `.m` files (single source of truth), so tmdl M == `.m` M.
SQL uses `[bracket]` identifiers only (no embedded `"`), so the M string needs no escaping.
...
twin-divergence analysis against report 09, and the "faithful-but-surprising" items to brief
Rohit on — most importantly the **Sales Orders 12-plants-in / 6-plants-out join** (page 3's
`SO7` filter names 12 branches but `Item_Information8` names 6, and the join behaves as an inner
join, so 6 are silently dropped). That is faithful to Cognos and **the item most likely to be
reported as a bug.** This file records *how the thing was built and why*; that file records
*what is still owed*.
