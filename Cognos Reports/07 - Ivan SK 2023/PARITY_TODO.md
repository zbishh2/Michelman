# Parity TODO — 07 - Ivan SK 2023
_Source of truth: PBIP. Structural twin of `09 - Ivan FC 2023`._

## Status — 2026-07-09

**A "last refreshed" timestamp was added to this report's `PBIP\`. It is in the PBIP only — it is not in
the PBIX.** `FINAL - for handover\1 - Ivan SK 2023.pbix` was opened and inspected on 2026-07-09: it
contains no `Last Refreshed` table and no `card` visual. Someone must open
`PBIP\1 - Ivan SK 2023.pbip` in Power BI Desktop and re-save / publish before any user sees this. Nothing
...
| 6.4 | **Hardcoded Work Orders start date `2026-03-01`.** Faithful to Cognos, but it is a literal that silently ages. | Cognos `Work Orders` detailFilter `[Start Date]>=2026-03-01`; `Work_Orders.m` renders `>= '2026-03-01'`. | LOW (informational) | None — parity is correct. Flag to the business as a maintenance item, since Cognos had the same landmine. |
...
| (b) Cognos render-DISTINCT missing → `Work_Orders` over-counts | **FIXED** | `Work_Orders.m:67` `SELECT DISTINCT`. Also present where needed in `Safety_Stock_HP.m:36` and `Sales_Order_Summary.m:99` (Item Information CTE) plus the outer `GROUP BY`. |
...
- **Sales Orders' 12-plant filter yields only 6 plants.** The `Sales Orders` sub-query filters `SDMCU IN ('AUBA','AUB2','SING','SNG4','MUM3','SHAN','CINC','CIN2','CIN4','BARC','CIND','CINR')`, but the `Item Information` sub-query is fixed at the 6 APAC/EMEA plants, and the join on `Branch_Plant` behaves as an inner join. The six Americas/BARC plants are therefore dropped. Faithful to Cognos and documented at `Sales_Order_Summary.m:31`. **This is the item most likely to be reported as a bug.**
...
- **Disabled Cognos filters correctly excluded.** Three queries (`Inventory`, `Inventory - New`, `Safety Stock - New`) each carry a second `[Bulk Item] in (...)` filter marked `use="prohibited"` — in this report it is a leftover 15-item **FC** whitelist. Set-differencing the active Cognos lists against the `.m` `IN (...)` lists gives an exact match with **zero** prohibited items leaked (99/99, 13/13, 21/21). Had it been ANDed in, these pages would return 0 rows.
...
Observation on the *Cognos source* (not a rebuild defect): the `use="prohibited"` filter in this report is 09's FC bulk-item whitelist, while in 09 the prohibited filter is a 15-item subset of its own 18-item active list. This is strong evidence that the SK report was cloned from the FC report inside Cognos. It has no effect on either rebuild, since prohibited filters are correctly ignored in both.

The genuine, intended content differences between the twins (all faithfully carried through): SK's bulk-item whitelists are three distinct lists (99 / 13 / 21 entries) where FC reuses one 31-literal / 18-distinct list everywhere; SK's Inventory branch list has 9 plants vs FC's 6; SK's Sales Orders branch list has 12 plants vs FC's 6; SK's Work Orders component filter is the `PR*` family with `Start Date >= 2026-03-01`, FC's is the `BRIJS*` family with `>= 2025-11-01`.
...
GUIDs, 09's `Last Refreshed.tmdl` carries `annotation SummarizationSetBy = Automatic` on both columns and
07's does not. That annotation is Power BI Desktop bookkeeping with no semantic effect — same data types,
same `summarizeBy: none`, same `formatString`. It will normalise itself the first time each file is
re-saved from Desktop.
...
- [ ] 8.x Brief Rohit on the five "faithful-but-surprising" items above, especially the Sales Orders 12-plants-in / 6-plants-out join — MED (handover risk, not a code defect) — ~20 min
