// ============================================================================
// Report 18  ·  1 - Singapore Warehouse Inv 2025  ·  page table: Lot Status
// Cognos query object: Lot Status_2  (DW_LEGACY "Inventory On Hand Star Schema")
...
let
    ...
    Data = Value.NativeQuery(
        ...
        WHERE d.d >= CAST(GETDATE() - (365.0/2) AS date)
          AND d.d <= CAST(DATEADD(DAY, -1, GETDATE()) AS date)
          AND (DATEDIFF(DAY, '1900-01-07', d.d) % 7) IN (0, 2, 5)
          AND d.d <> '2025-05-07';
        ...
        FROM (
            SELECT
                ...
                CAST(DATEADD(DAY, -1, GETDATE()) AS date)          AS [DATE]
            FROM #dates dt
            ...
            WHERE (snap.QuantityOnHandPrimaryUOM > 0 OR snap.ItemCostSKey <> -1)
              ...
              AND (LTRIM(RTRIM(ISNULL(snap.LotStatusCode,''))) NOT IN ('', '-') OR snap.QuantityOnHandPrimaryUOM = 0)
        ) r
        WHERE r.[_QOH] > 0
