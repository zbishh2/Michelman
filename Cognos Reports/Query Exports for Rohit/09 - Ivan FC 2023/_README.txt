...
Power BI file : 1 - Ivan FC 2023.pbix
                Standalone, 5 pages. Structural twin of report 07 - Ivan SK 2023; differs only in the filter literals (FC vs SK).
...
Queries in this folder (5 .txt):
  ...
  Inventory.txt
     ...
     global branches, with hard commit and KG/LB.
  ...
  Sales_Order_Summary.txt
     ...
     Tables: F4211, F42140, F0101, F0116, F0006, F4201, F4102, F4101, F554101;
     country UDC decode uses PRODCTL.F0005.

  Inventory_HP.txt
     Page 4 "Inventory HP" (Cognos query object "Inventory - New"). On-hand restricted to available lot
     statuses (blank or T/B/Q/H), fewer columns.
     Tables: F4102, F4101, F554101, F41021.

  Safety_Stock_HP.txt
     Page 5 "Safety Stock HP" (Cognos query object "Safety Stock - New"). SELECT DISTINCT safety stock
     (F4102.IBSAFE) per item/branch with an LB conversion.
...
filters sitting beside ACTIVE `[Bulk Item] in (...)` filters on the same column.
The two lists overlap but are not identical. The disabled copies never reached the
Cognos generated SQL, so they are not applied - and we match the generated SQL,
which is the record of what actually ran. Do not read the disabled list as this
report's scope.
