==============================================================================
REPORT : 10 - Ivan SFC2023 Forecast
==============================================================================

Power BI file : 1 - Ivan FC 2023 Forecast.pbix
                Standalone, 2 pages. Structural twin of report 08.
...
Queries in this folder (2 .txt):
  ...
  Forecast.txt
     ...
     *** UNVALIDATED - BEST EFFORT. *** Cognos read the DW_LEGACY Oracle warehouse
     ...
     UNCONFIRMED and are marked inline with `-- TODO verify`. See BUILD.md ->
     "ASSUMPTIONS & VALIDATION RISKS".
     Tables: F3460, F4101, F554101, F0006, F0101, F42140.

  Sales_History.txt
     Page "Sales History". Shipped/booked sales history by customer, item and due date, with KG/LB.
     Reads F4211 UNION ALL F42119 (completed status-999 lines are purged from F4211 into F42119).
     Rebuilt off base JDE tables; the Cognos original reads DW_LEGACY (ORDER_ACTIVITY star schema).
     Tables: F4211, F42119, F42140, F0101, F0116, F0006, F554101; country UDC decode uses PRODCTL.F0005.
...
The evidence is in the repo. `Cognos Reports\Ivan Reports\Ivan FC 2023 Forecast.xlsx`
...
908 rows. The report ran. The forecast query returned nothing.
...
The `-- TODO verify` markers are not equally valuable. In descending order:
  ...
  3. `RELOAD_KEY = 'N'` - a conscious, documented omission, not a silent drop.
     ...
     in the source `Forecast.m` at line 57. It corresponds to a Cognos filter
