...
- The jumpbox is the network execution surface for `SSASPROD`, `EDWPROD`, `EDWDEV`, `ODSPROD`,
  and `ODSDEV`. It is RDP-only; WinRM is not an alternative.
...
4. Let Zack sign in to and unlock the jumpbox. An agent can control the already-authenticated RDP
   session afterward.
...
## 5. Jumpbox probing with SSMS

Use SSMS on the jumpbox for arbitrary, read-only DAX against on-premises SSAS.

1. Connect SSMS to **Analysis Services**, server `SSASPROD`.
2. Select the exact database (`BIQLTabular` or `BIQLTabular_ISH`).
3. Start with a narrow `EVALUATE` query for one known business key and one date.
4. Add diagnostic row counts and distinct counts before expanding scope.
5. Paste the result grid back into the task or save it under the report's probe folder.
...
The local machine cannot reliably reach SSASPROD directly. A local connection timeout is a topology
constraint, not evidence that SSASPROD is down.

## 6. Windows computer control

Computer Use can operate the already-open RDP, Power BI Desktop, Report Builder, Excel, and SSMS
windows. Use it as an interaction layer, not as a substitute for file or command-line automation.

- Select exactly one returned target window before acting.
- For jumpbox work, target the Remote Desktop window; applications inside the remote desktop are
  painted inside that window and are not separate local windows.
...
- Prefer keyboard navigation for remote editors and grids.
- If reliable text injection into an RDP editor fails, give Zack a probe to paste and run. This is
  faster and safer than repeated blind typing.
...
connection to on-premises SSAS. PPU XMLA has the same live-model limitation. Query those models in
SSMS on the jumpbox.
...
source freshness, gateway behavior, or jumpbox-only connectivity.
...
For on-premises SSAS Live reports, use the published report for render/export testing and the
jumpbox SSMS connection for arbitrary DAX. Publishing one thin report per perspective improves field
discovery but does not turn the live proxy into a REST/XMLA-queryable Power BI model.
...
1. Authenticate Fabric, Cognos, and the jumpbox through the human handoffs.
...
6. Validate locally where possible; run network-bound refreshes and SSAS probes on the jumpbox.
