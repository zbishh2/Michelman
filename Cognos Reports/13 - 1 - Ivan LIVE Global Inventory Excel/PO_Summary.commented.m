// ============================================================================
// Report 13 - 1 - Ivan LIVE Global Inventory Excel   (page 2 of 6: "PO")
// QUERY: PO Summary  ->  Cognos list "List3", query object "PO Summary"
// SOURCE SQL: "PO Summary.1.sql"
//
// Columns (RENDERED order, 29 - headers verbatim; note join-dup names Branch Plant1,
//   2nd Item Number1 rendered in dark blue in Cognos - reproduce names as-is):
//   Company | Branch Plant | 2nd Item Number | Purchase Order Number | Reference 2 |
...
//   Vendor Name | Freeze Code Flag | Branch Plant1 | Global Bulk Item | Bulk Item |
//   2nd Item Number1 | Commodity Class | Sub Class | Master Planning Family | GL Category |
//   Primary Supplier | Safety Stock | Leadtime Level
//
// SOURCE: ODSPROD / "ODS" / PRODDTA (JDE), SQL Server. Native T-SQL, folds.
//
// LOGIC (faithful to "PO Summary.1.sql"): Item master (Item6) INNER JOIN open POs
//   (Purchase_Orders5) on Branch Plant + 2nd Item. Cognos's LEFT OUTER JOIN + WHERE
//   equality is an inner join.
//     * Item6                = DISTINCT item-branch attributes (F4102/F554101/F4101).
//     * Purchase_Orders5     = F4311 PO detail joined to F0101 (vendor name), grouped,
//                              filtered Promised Date >= today-365 AND PO Type = 'OP'.
//   Two aggregation layers kept exactly as Cognos generated them (idempotent re-SUM).
//
...
//   NVL(trim(PDUNCD),'-') -> ISNULL(NULLIF(LTRIM(RTRIM(PDUNCD)),''),'-') (Oracle trim(spaces)=NULL);
//   PDDCTO=N'OP' -> ='OP'; x/10000 -> x/10000.0; to_date(sysdate)-365 ->
//   DATEADD(DAY,-365,CAST(GETDATE() AS date)); no ORDER BY (folded).
//   PARITY: Cognos sums Vendor Code (an address-book number) - kept as SUM.
//   Sort: none in the Cognos "PO" list (no <sortList>) -> leave the visual unsorted or
//   sorted by natural key; see BUILD.md.
//   Numeric identifiers (Purchase Order Number, Vendor Code, Primary Supplier) must be
//   summarizeBy:none in the visual - see BUILD.md.
// Expected rows to xlsx "PO_2": 7,098 (as-of capture; live counts drift).
// ============================================================================
let
