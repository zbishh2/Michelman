// Power Query-style sort + filter popover, anchored under the clicked header.
// Value checklist ("main" view) or typed conditions ("conditions" view).
...
export const HeaderMenu: React.FC<HeaderMenuProps> = ({
    ...
    useEffect(() => {
        ...
        // The visual is sandboxed in an iframe, so clicks on the report canvas
        // never reach our mousedown handler. A window blur means focus left the
        // visual — close, unless focus is still inside the popover.
        const onBlur = () => {
            window.setTimeout(() => {
    ...
    return (
        <div className="rle-hdr-menu" ref={ref} style={{ left: pos.left, top: pos.top, maxHeight: maxH }} onContextMenu={(e) => e.preventDefault()}>
            {view === "main" ? (
                <>
                    <div className="rle-hdr-menu-section">
                        <button type="button" className={"rle-hdr-menu-item" + (sortDirection === "asc" ? " active" : "")} onClick={() => { onSort("asc"); onClose(); }}>▲ Sort ascending</button>
                        <button type="button" className={"rle-hdr-menu-item" + (sortDirection === "desc" ? " active" : "")} onClick={() => { onSort("desc"); onClose(); }}>▼ Sort descending</button>
                        <button type="button" className="rle-hdr-menu-item" disabled={!sortDirection} onClick={() => { onClearSort(); onClose(); }}>Clear sort</button>
                    </div>
                    <div className="rle-hdr-menu-section">
                        <button type="button" className="rle-hdr-menu-item" disabled={!currentFilter} onClick={() => { onApply(null); onClose(); }}>Clear filter</button>
                        <button type="button" className="rle-hdr-menu-item" disabled={!hasAnyFilter} onClick={() => { onClearAll(); onClose(); }}>Clear all filters</button>
                        <button type="button" className="rle-hdr-menu-item" onClick={() => setView("conditions")}>
                            {dataType === "number" ? "Number" : dataType === "date" ? "Date" : "Text"} filters <span className="rle-hdr-chevron">▸</span>
                        </button>
                    </div>
                    <div className="rle-hdr-filter">
                        <input
                            ref={searchRef}
                            className="rle-hdr-filter-search"
                            value={valueSearch}
                            placeholder="Search"
                            onChange={(e) => setValueSearch(e.target.value)}
                        />
                        <div className="rle-hdr-filter-scroll" ref={scrollWrapRef}>
                            <ScrollPane>
                                <ul className="rle-hdr-filter-list" ref={listRef}>
                                    <li className="rle-hdr-filter-opt" onClick={toggleSelectAll}>
                                        <span className={"rle-hdr-check" + (allFilteredChecked ? " on" : someFilteredChecked ? " some" : "")}>
                                            {allFilteredChecked ? "✓" : someFilteredChecked ? "–" : ""}
                                        </span>
                                        <span className="rle-hdr-filter-label"><b>(Select all)</b></span>
                                    </li>
                                    {visible.map((v) => (
                                        <li key={v} className="rle-hdr-filter-opt" onClick={() => toggleValue(v)}>
                                            <span className={"rle-hdr-check" + (checked.has(v) ? " on" : "")}>{checked.has(v) ? "✓" : ""}</span>
                                            <span className="rle-hdr-filter-label">{labelForValue(v)}</span>
                                        </li>
                                    ))}
                                    {filtered.length === 0 && <li className="rle-hdr-filter-opt rle-hdr-filter-empty">No matches</li>}
                                    {filtered.length > VALUE_RENDER_CAP && (
                                        <li className="rle-hdr-filter-opt rle-hdr-filter-empty">
                    ...
                    <div className="rle-hdr-menu-actions">
                        <button type="button" className="rle-btn" onClick={handleOk}>OK</button>
                        <button type="button" className="rle-btn" onClick={onClose}>Cancel</button>
                    </div>
                </>
            ) : (
                <>
                    <div className="rle-hdr-menu-section">
                        <button type="button" className="rle-hdr-menu-item" onClick={() => setView("main")}>
                            <span className="rle-hdr-chevron">◂</span> Back
                        </button>
                    </div>
                    <div className="rle-hdr-cond">
                        <select className="rle-hdr-cond-op" value={op} onChange={(e) => setOp(e.target.value)}>
                            {ops.map(([value, text]) => <option key={value} value={value}>{text}</option>)}
                        </select>
                        {needsValue && (
                            <input
                                className="rle-hdr-cond-val"
                                type={inputType}
                                value={val}
                                placeholder="Value"
                                autoFocus
                        ...
                        {needsValue && needsTwo && (
                            <>
                                <span className="rle-hdr-cond-and">and</span>
                                <input
                                    className="rle-hdr-cond-val"
                                    type={inputType}
                                    value={valTo}
