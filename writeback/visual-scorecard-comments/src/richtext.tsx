...
function toNodes(toks: Tok[], opts: RichTextOptions, keyBase: string): React.ReactNode[] {
    return toks.map((t, i) => {
        ...
        switch (t.kind) {
            ...
            case "code": return <code className="scb-code" key={key}>{t.text}</code>;
            ...
            case "url": return opts.launchUrl
                ...
                >{t.text}</a>
                : <span className="scb-a-plain" key={key}>{t.text}</span>;
        }
    });
}
...
export function renderRich(body: string, opts: RichTextOptions = {}): React.ReactNode {
    return parseBlocks(body).map((b, i) => {
        switch (b.kind) {
            case "blank": return <div className="scb-gap" key={i} />;
            case "p": return <div className="scb-line" key={i}>{toNodes(b.toks, opts, String(i))}</div>;
            case "ul": return (
                <ul className="scb-list" key={i}>
                    {b.items.map((it, j) => <li key={j}>{toNodes(it, opts, `${i}-${j}`)}</li>)}
                </ul>
            );
            case "ol": return (
                <ol className="scb-list" key={i}>
                    {b.items.map((it, j) => <li key={j}>{toNodes(it, opts, `${i}-${j}`)}</li>)}
                </ol>
            );
        }
