...
export class Visual implements IVisual {
    ...
    // Push column filters to the report page. We own `general.filter` as write-
    // only: clear the previous set, then merge the new one. A signature dedupe
    // avoids re-pushing on every dataView refresh and breaks the
    // applyJsonFilter → update() → push feedback loop (DR's infinite-loop lesson).
    ...
    public update(options: VisualUpdateOptions): void {
        ...
        const dv = pickGridDataView(options.dataViews, "orderLineId");
        ...
        const commentsPosition = this.formattingSettings.tableCard.commentsPosition.value;
        ...
        try {
            ...
            console.error("Order Line Comments visual render error:", err);
