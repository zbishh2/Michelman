...
//
// This lives apart from the grid's dataView on purpose. A scalar measure sitting alongside the
// grid's ~20 group-by columns forces the formula engine to materialize every order-line group
// instead of letting the storage engine scan; measured on the OTIF model, the identical query
// ran in 85ms without it and died with "not enough memory" after 72s with it. capabilities.json
// therefore declares a SECOND dataViewMapping selecting only the Current User role, so the host
// issues a separate one-row query for it.
//
