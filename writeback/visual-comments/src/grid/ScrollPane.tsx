// Custom overlay scrollbars. Native scrollbars trigger a Chromium iframe-
// transform blur in the PBI sandbox; hide them and render our own thin thumbs.
// Ported verbatim from the DeliveryReliability visual.
...
export const ScrollPane: React.FC<{ children: React.ReactNode; onScroll?: (el: HTMLDivElement) => void }> = ({ children, onScroll }) => {
    ...
    return (
        <div className="olc-scroll-pane">
            <div
                className={"olc-scroll-inner" + (showH ? " has-h" : "") + (showV ? " has-v" : "")}
                ref={innerRef}
            >{children}</div>
            {showH && (
                <div className="olc-htrack" onMouseDown={onHTrackClick}>
                    <div
                        className={"olc-hthumb" + (dragH ? " dragging" : "")}
                        style={{ left: hThumb.left, width: hThumb.size }}
                        onMouseDown={startHDrag}
                    />
                </div>
            )}
            {showV && (
                <div className="olc-vtrack" onMouseDown={onVTrackClick}>
                    <div
                        className={"olc-vthumb" + (dragV ? " dragging" : "")}
                        style={{ top: vThumb.top, height: vThumb.size }}
