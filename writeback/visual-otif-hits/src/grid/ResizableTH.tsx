// Header cell: resize handle (persists width), drag-drop reorder, and a
// click-to-open sort/filter menu. Ported from the DeliveryReliability visual.
...
export const ResizableTH: React.FC<ResizableTHProps> = ({ label, width, numeric, onResize, onResizeEnd, dragKey, onColumnDrop, sortDirection, onOpenMenu, filterActive, stickyLeft, frozenBoundary, onMeasured }) => {
    ...
    return (
        <th
            ...
            className={[
                numeric ? "num" : "",
                dragKey ? "olc-draggable-th" : "",
                stickyLeft != null ? "olc-frozen-col" : "",
                frozenBoundary ? "olc-frozen-boundary" : "",
            ].filter(Boolean).join(" ")}
            style={style}
            draggable={!!dragKey}
            onDragStart={(e) => {
        ...
        >
            <span className="olc-th-inner">
                <span className="olc-th-label" title={label}>{label}</span>
                {sortDirection && <span className="olc-sort-indicator">{sortDirection === "asc" ? " ▲" : " ▼"}</span>}
                {filterActive && <span className="olc-th-filter-icon active" title="Filtered">⏷</span>}
                {!filterActive && <span className="olc-th-menu-caret">▾</span>}
            </span>
            <div className="olc-resizer" draggable={false} onMouseDown={onMouseDown} />
        </th>
    );
};
