...
// dropped that form, so Desktop resolved to no identity at all and the admin seeded in D1
// as `desktop-pkqsmbt\zack` got a read-only grid — with a banner blaming an unbound field
// well. Accept both shapes; anything else is still rejected so a stray text column bound
// to the field well cannot masquerade as an identity.
...
// Grid values are free text and several carry embedded newlines (the reason-code
// Long Description is written as multi-line "i.e:" notes). A raw "\t"/"\n" join
// therefore loses the table shape the moment Excel parses it: every newline
// INSIDE a cell starts a new spreadsheet row, which is what shifted the long
// descriptions down into column A. Two defences, both needed:
