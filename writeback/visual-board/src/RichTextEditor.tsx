...
const FORMAT_BUTTONS: { fmt: Fmt; label: string; title: string; cls: string }[] = [
    { fmt: "bold", label: "B", title: "Bold  (Ctrl+B)", cls: "obc-tb-bold" },
    { fmt: "italic", label: "I", title: "Italic  (Ctrl+I)", cls: "obc-tb-italic" },
    { fmt: "strikethrough", label: "S", title: "Strikethrough", cls: "obc-tb-strike" },
];
...
const Toolbar: React.FC = () => {
    ...
    const btn = (key: string, label: string, title: string, cls: string, on: boolean, run: () => void) => (
        <button
            type="button"
            key={key}
            className={"obc-tb " + cls + (on ? " obc-tb-on" : "")}
            title={title}
            aria-pressed={on}
    ...
    return (
        <>
            ...
            {btn("ol", "1.", "Numbered list", "obc-tb-ol", false,
...
export const RichTextEditor = React.forwardRef<RichTextEditorHandle, RichTextEditorProps>((props, ref) => {
    ...
    return (
        <div className={"obc-rte" + (className ? " " + className : "")}>
            <LexicalComposer initialConfig={initialConfig}>
                <div className="obc-toolbar">
                    <Toolbar />
                    {props.children}
                </div>
                    ...
                    <RichTextPlugin
                        contentEditable={
                            <ContentEditable
                                ...
                                placeholder={<div className="obc-placeholder">{placeholder}</div>}
                            />
                        }
